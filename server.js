...
function verifyPassword(password, stored) {
  ...
  if (!stored.startsWith('scrypt$')) {
    // Legacy plaintext is supported once so an existing installation can be migrated.
    const a = Buffer.from(password); const b = Buffer.from(stored); return a.length === b.length && crypto.timingSafeEqual(a, b);
  }
...
async function ensureSchema() {
  ...
  try {
    ...
    // Add missing columns to an older carnival_state table created by a previous version.
    const columns = [
      ['users', "JSONB NOT NULL DEFAULT '[]'::jsonb"],
      ['zones', "JSONB NOT NULL DEFAULT '[]'::jsonb"],
      ['pops', "JSONB NOT NULL DEFAULT '[]'::jsonb"],
    ...
    for (const [name, type] of columns) {
      await client.query(`ALTER TABLE public.carnival_state ADD COLUMN IF NOT EXISTS ${name} ${type}`);
    }
...
async function bootstrapUsers() {
  ...
  for (const u of defaults) {
    ...
    await pool.query(
      `INSERT INTO public.app_users (id, username, role, password_hash) VALUES ($1,$2,$3,$4)`,
      [u.id, u.username, u.role, scryptHash(password)]
    );
  }

  // Keep the application state user list aligned with app_users without exposing hashes.
  ...
  state.users = users.rows.map(u => ({
    id: u.id, username: u.username, role: u.role, ...(u.permissions ? { permissions: u.permissions } : {})
  }));
...
async function importLegacyTablesIfStateEmpty() {
  ...
  try {
    ...
    const mappings = [
      ['zones', 'zones'], ['pops', 'pops'], ['olts', 'olts'], ['ports', 'ports'],
      ['first_splitters', 'firstSplitters'], ['second_splitters', 'secondSplitters'], ['clients', 'clients']
    ];
    for (const [table, key] of mappings) {
      ...
      try {
        ...
        console.warn(`Legacy table import skipped for ${table}: ${e.message}`);
      }
    }
    if (result.zones.length || result.pops.length || result.olts.length || result.ports.length || result.firstSplitters.length || result.secondSplitters.length || result.clients.length) {
...
async function syncAppUsersFromState(state) {
  // Admin-only user management is handled here. Blank password means keep existing password.
  ...
  // Mirror the UI's delete-user behavior. Keep the built-in admin account.
...
async function syncState(state, user) {
  ...
  if (user.role !== 'admin') {
    // Prevent non-admins from changing application accounts.
    const old = await readState();
    normalized.users = old.users;
  }
  await writeState(normalized);
  if (user.role === 'admin') await syncAppUsersFromState(normalized);

  // Re-read users from the authoritative auth table so state and login stay consistent.
  const users = await pool.query('SELECT id, username, role, permissions FROM public.app_users ORDER BY username');
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    const u = q.rows[0];
    let valid = verifyPassword(password, u.password_hash);
    ...
    sessions.set(token, { user: { id: u.id, username: u.username, role: u.role, permissions: u.permissions || undefined }, expiresAt: Date.now() + SESSION_MS });
    res.json({ ok: true, token, user: { id: u.id, username: u.username, role: u.role, permissions: u.permissions || undefined } });
  } catch (e) {
